...
%Get the value of the step input from the slider%        
        ...
        t1 = 0:0.1:5;
        de1 = stepval*ones(size(t1));
        de2 = stepval*zeros(size(t1));
        de = [de1, de2];
        de = de(1, 1:101);
%Run the simulation%             
...
%Set the axis limits for the step response plot%
        ...
        title ('Risposta Sistema')
      	ylabel ('Angolo Pitch (rad)')
      	xlabel ('Tempo (sec)')
	   ...
	   if man == 1
	      pause
           ...
           set(Q,'YData',[ywrtop(i) ywrbot(i)]);
      	  ...
      	  axes(handles.axes1)
      	   if plotval == 0
      	      plot([t(i),t(i+1)],[theta(i),theta(i+1)])
              hold on
      	   end 
...
stepval=get(handles.stepslider,'value');
set(handles.text2,'string',sprintf('%6.4f',stepval*180/3.14));
